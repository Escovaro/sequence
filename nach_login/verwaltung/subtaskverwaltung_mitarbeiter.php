                ...
                <div class="container-fluid text-nowrap verwaltungs-container table-responsive">
                    ...
                    <table class="table table-striped w-auto" id="myTable">
                        ...
                        ?>
                                ...
                                $sql0 = "SELECT position FROM mitarbeiter WHERE mitarbeiterid = '".$_SESSION['mitarbeiterid']."'";
                                $position = mysqli_query($conn, $sql0) or die("database error:". mysqli_error($conn));

                                if ($position == '1' || $position == '2'){
                                    $sql1 = "SELECT projektid FROM arbeiteteannorm WHERE mitarbeiterid = '".$_SESSION['mitarbeiterid']."'";                            
                                    while($record = mysqli_fetch_assoc($resultset)){
                                        $projektid = $record['projektid'];
                                                                            
                                        $sql2 = "SELECT * FROM subtask WHERE projektid = $projektid ORDER BY subtaskid ASC";
                                    ...
                                    $sql2 = "SELECT * FROM subtask ORDER BY subtaskid ASC";
                                    $resultset = mysqli_query($conn, $sql2) or die("database error:". mysqli_error($conn));
                                    while( $record = mysqli_fetch_assoc($resultset) ) {
                                        $stdate1 = date('d.m.y',strtotime($record['erstelldatum']));
                                        $stdate2 = date('d.m.y',strtotime($record['deadline']));
